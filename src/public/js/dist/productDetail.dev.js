...
window.addEventListener('DOMContentLoaded', function () {
  ...
  function productosRelacionadosSlider() {
    var $nextProduct = document.querySelector('.btnSig');
    var $prevProduct = document.querySelector('.btnAnt');
    var $productsRelacionados = document.querySelector('.productsSliderRelacionados');
    console.log($productsRelacionados.children.length); //Los muestro como vista inicial
    ...
    function display_Initial_ProductsView(width, howManyProducts) {
      if (width) {
        for (var i = 0; i < $productsRelacionados.children.length; i++) {
