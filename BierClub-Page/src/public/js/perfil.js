...
document.addEventListener("DOMContentLoaded", () => {
  ...
  $formModificarUser.addEventListener("submit", (e) => {
    ...
    let nuevaContraseniaCondicion =
      $nuevaContrasenia.value.length < 8 ||
      $email.value == $nuevaContrasenia.value;
    let confirmarContraseniaCondicion =
      $confirmarContrasenia.value.length < 8 ||
      confirmarContrasenia.value != $nuevaContrasenia.value;
    ...
    validacionDeCampos(
      $antiguaContrasenia,
      antiguaContraseniaCondicion,
      "error-total",
      "* La contraseña debe tener al menos 8 caracteres"
    );
    validacionDeCampos(
      $nuevaContrasenia,
      nuevaContraseniaCondicion,
      "error-total",
      "* La contraseña debe tener al menos 8 caracteres y ser distitnta del email"
    );
    validacionDeCampos(
      $confirmarContrasenia,
      confirmarContraseniaCondicion,
      "error-total",
      "* Debe ser igual a contraseña nueva"
    );
